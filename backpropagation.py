...
class NeuralNetwork:
    ...
    def __init__(self, num_inputs, num_hidden, num_outputs, hidden_layer_weights = None,  output_layer_weights = None):
        ...
        self.init_weights_from_inputs_to_hidden_layer_neurons(hidden_layer_weights)#Se inicializan los pesos de la capa oculta
        self.init_weights_from_hidden_layer_neurons_to_output_layer_neurons(output_layer_weights)#Se inicializan los pesos de la capa de salida
    ...
    def feed_forward(self, inputs):
        hidden_layer_outputs = self.hidden_layer.feed_forward(inputs)#Calcula las salidas de la capa oculta
        #print("Salidas capa oculta:",hidden_layer_outputs)
        output_layer_outputs=self.output_layer.feed_forward(hidden_layer_outputs)#Calcula las salidas de la capa de salida
    ...
    def train(self, training_inputs, training_outputs):
        ...
        for h in range(len(self.hidden_layer.neurons)):
            ...
            for o in range(len(self.output_layer.neurons)):
                d_error_wrt_hidden_neuron_output += errorescapasalida[o] * self.output_layer.neurons[o].weights[h]
            # ∂E/∂zⱼ = dE/dyⱼ * ∂zⱼ/∂
            #   Sumatoria de (error de salida* peso de la salida)* salida(1-salida)
            #   Σ(Spk*Wkj)*Ypj(1-Ypj)
            errorescapaoculta[h] = d_error_wrt_hidden_neuron_output * self.hidden_layer.neurons[h].calculate_pd_total_net_input_wrt_input()
        ...
        for o in range(len(self.output_layer.neurons)):
            for w_ho in range(len(self.output_layer.neurons[o].weights)):
                ...
                pd_error_wrt_weight =self.LEARNING_RATE* errorescapasalida[o] * self.output_layer.neurons[o].calculate_pd_total_net_input_wrt_weight(w_ho)
                # Δw = α * ∂Eⱼ/∂wᵢ
        ...
        for h in range(len(self.hidden_layer.neurons)):
            for w_ih in range(len(self.hidden_layer.neurons[h].weights)):
                ...
                # Δwji (t+1)=Spi* Xpi
                # delta de capa oculta=Error de capa de oculta*entrada net
                pd_error_wrt_weight =  self.LEARNING_RATE*errorescapaoculta[h] * self.hidden_layer.neurons[h].calculate_pd_total_net_input_wrt_weight(w_ih)

                # Δw = α * ∂Eⱼ/∂wᵢ
                #Actualizar pesos de capa oculta
                # wji(t+1)=wji(t)+Δwji
                # wji(t+1)=(wji(t+1)-wji(t))+Δwji
                self.hidden_layer.neurons[h].weights[w_ih]=self.hidden_layer.neurons[h].weights[w_ih-1] + pd_error_wrt_weight
...
def cargarTrainingPrueba():
    lista = os.listdir(os.path.dirname(os.path.abspath(__file__)) + "\\inputsdesconocidas\\")
    for filename in lista:
        trainsetElement = []
        currentPath = os.path.dirname(os.path.abspath(__file__)) + "\\inputsdesconocidas\\"
        index = lista.index(filename)
        outputNN = []
        inputNN = []
        if(index % 2 == 0):
